...
export interface HeroPhysicsStageHandle {
  handleChickenDrop: (clientX: number, clientY: number, velocity: number) => void;
  handleChickenWalk: (chickenX: number, chickenY: number) => void;
  chickenExpression: ChickenExpression;
}
...
const INITIAL_LETTERS: Omit<LetterData, 'posX' | 'posY' | 'wobble'>[] = [
  // Word 0: Aditya
  { id: 'i1', char: 'A', wordIdx: 0, letterIdx: 0 },
  { id: 'n1', char: 'D', wordIdx: 0, letterIdx: 1 },
  { id: 't1', char: 'I', wordIdx: 0, letterIdx: 2 },
  { id: 'a1', char: 'T', wordIdx: 0, letterIdx: 3 },
  { id: 'n2', char: 'Y', wordIdx: 0, letterIdx: 4 },
  { id: 'a4', char: 'A', wordIdx: 0, letterIdx: 5 },

  // Word 1: Permana
  { id: 's1', char: 'P', wordIdx: 1, letterIdx: 0 },
  { id: 'r1', char: 'E', wordIdx: 1, letterIdx: 1 },
  { id: 'i2', char: 'R', wordIdx: 1, letterIdx: 2 },
  { id: 'm1', char: 'M', wordIdx: 1, letterIdx: 3 },
  { id: 'a2', char: 'A', wordIdx: 1, letterIdx: 4 },
  { id: 'y1', char: 'N', wordIdx: 1, letterIdx: 5 },
  { id: 'a3', char: 'A', wordIdx: 1, letterIdx: 6 },
];
...
interface HeroPhysicsStageProps {
  ...
  onChickenExpressionChange?: (expression: ChickenExpression) => void;
}

export const HeroPhysicsStage = forwardRef<HeroPhysicsStageHandle, HeroPhysicsStageProps>(
  ({ onSplash, onRipple, onChickenExpressionChange }, ref) => {
    ...
    const chickenImpactTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const lastRippleTimeRef = useRef<number>(0);
    const returnTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

    const updateChickenExpression = useCallback((expr: ChickenExpression) => {
      setChickenExpression(expr);
      if (onChickenExpressionChange) {
        onChickenExpressionChange(expr);
      }
    }, [onChickenExpressionChange]);
    ...
    const handleChickenDrop = (clientX: number, clientY: number, velocity: number) => {
      onSplash(clientX, clientY, velocity);

      const didHit = resolveCollisions('chicken', clientX, clientY, velocity * 1.3);

      if (didHit) {
        updateChickenExpression('impact');
        if (chickenImpactTimeoutRef.current) clearTimeout(chickenImpactTimeoutRef.current);
        chickenImpactTimeoutRef.current = setTimeout(() => {
          updateChickenExpression('normal');
        }, 1100);
      } else {
        updateChickenExpression('normal');
      }
    ...
    const handleChickenWalk = (chickenX: number, chickenY: number) => {
      ...
      if (now - lastRippleTimeRef.current > 380) {
        ...
        onRipple(chickenX, chickenY, 45);
      }

      // Early exit if chicken is nowhere near the letters container
      if (containerRef.current) {
        const cRect = containerRef.current.getBoundingClientRect();
        if (
          chickenY < cRect.top - 120 ||
          chickenY > cRect.bottom + 120 ||
          chickenX < cRect.left - 120 ||
          chickenX > cRect.right + 120
        ) {
          return;
        }
      }
      ...
      setLetters((prev) => {
        ...
        const updated = prev.map((letter) => {
          ...
          const dx = letterCenterX - chickenX;
          const dy = letterCenterY - chickenY;
    ...
    const renderLetter = (letter: LetterData) => {
      ...
      return (
        ...
        >
          ...
          >
            {/* Grassy / Hay Shadow */}
            <div className="absolute -bottom-3 left-1 right-1 h-3 bg-[#15803d]/30 rounded-full blur-[3px] transform scale-y-50" />

            {/* Main Letter with high-performance CSS layered text shadow */}
            <span
              className="relative z-10 block text-[40px] min-[390px]:text-[48px] sm:text-7xl md:text-8xl font-black text-[#fffdf5] tracking-tight"
              style={{
                textShadow: '2px 2px 0px #15803d, 4px 4px 0px #78350f, 6px 6px 0px #1c1917, 0 8px 16px rgba(22,101,52,0.4)'
              }}
            >
              {letter.char}
            </span>
    ...
    return (
      <div ref={containerRef} className="relative z-20 w-full flex flex-col items-center justify-center">
